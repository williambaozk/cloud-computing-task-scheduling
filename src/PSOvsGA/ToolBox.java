...
public class ToolBox
{
	...
	public static void runSimulation_PSO(DatacenterBroker broker)
	{
		...
		long spendTimeGA=endGA-startGA;
		...
		System.out.println("This (GA) schedule plan takes "+spendTimeGA+"ms to train, takes "+finishTm+" ms to finish execution.");
		
		
		//由于每次执行PSO调度算法的调度结果都不同(由于PSO过程中加入了随机性,甚至可能比RR还差),以下3行代码是取n次调度方案来计算PSO结果的平均执行时间.
		int n=1;
		long startpso=System.currentTimeMillis();
		double avgRuntime=getAvgRuntimeByPSOscheduling(n,w,c1,c2,gmax)+1;
		long endpso=System.currentTimeMillis();
		long spendTimePSO=endpso-startpso;
		System.out.println("This (PSO) schedule plan takes "+spendTimePSO+"ms to train, takes "+avgRuntime+" ms to finish execution.");//模拟中从0.1开始运行，所以+0.1
