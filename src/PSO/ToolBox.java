...
public class ToolBox
{
	...
	public static void runSimulation_PSO(DatacenterBroker broker)
	{
		...
		//输出不同迭代次数的结果 迭代1-100次
		double[] avgRuntimes = new double[101];
		for(int i=1;i<=100;i++)
		{
			int ngmax=i;
			avgRuntimes[i]=getAvgRuntimeByPSOscheduling(n,w,c1,c2,ngmax);
		...
		for(int i=1;i<=100;i++)
		{
			System.out.print(String.format("%.2f\t",avgRuntimes[i]+0.1));
		}
...
//		}
//		System.out.println();
//		System.out.println(String.format("%.2f", getFitness(schedule)));
		
		int[] newschedule=schedule.clone();
		
		//计算pbest
		int[] pbest=getpbest(schedule);
		//int[] pbest=schedule.clone();
...
//		System.out.println();
		...
		//更新位置
		newschedule=updateposition(schedule,pbest,gbest,w,c1,c2);
		
		//如果更新位置之后的结果不如之前的则舍弃
		if(getFitness(newschedule)>getFitness(schedule))
		{
	...
	private static int[] getpbest(int[] schedule)
	{
		...
		for(int i=0;i<schedule.length;i++)
		{
			...
			for(int j=0;j<size;j++)
			{
				...
				if(!vmTasks.keySet().contains(temp[j]))
				{
					ArrayList<Integer> taskList=new ArrayList<Integer>();
					taskList.add(j);
					vmTasks.put(temp[j],taskList);
				}
				else
				{
					vmTasks.get(temp[j]).add(j);
				}
			}
	...
	private static int[] updateposition(int[] schedule,int[] pbest,int[] gbest,double w,double c1,double c2)
	{
	...
	public static void applyGAscheduling(int popSize,int gmax,double crossoverProb,double mutationRate)
	{
		...
		assignResourcesWithSchedule(schedule);
		System.out.println("结果——————————————————"+getFitness(schedule));
	}
